...
uses
  sysutils {$IFNDEF UNIX}, windows{$ENDIF};
...
type
  TSuperXMLState = (xsStart,                            // |
    ...
    xsElementDocTypeTryIntSubset, xsElementDocTypeIntSubset, xsElementDocTypeTryClose, xsElementDocTypeEat,                //
...
const
  ...
  publitteral =[#32, #13, #10, 'a'..'z', 'A'..'Z', '0'..'9', '-', '''', '"', '(', ')', '+', ',', '.', '/', ':', '=', '?', ';', '!', '*', '#', '@', '$', '_', '%'];
  ...
  begin
    ...
    if anobject = nil then
      parent.AsObject[name] := value
    else
    begin
      if (anobject.DataType = stArray) then
        anobject.AsArray.Add(value)
      else
      begin
...
redo:
    case FStack^.state of
      ...
      xsEatSpaces:
        if {$IFDEF UNICODE} (c < #256) and {$ENDIF} (AnsiChar(c) in spaces) then {nop}
      ...
      xsEscape:
        begin
          ...
          case c of
            'l':
              FStack^.state := xsEscape_lt;
            'g':
              FStack^.state := xsEscape_gt;
            'a':
              FStack^.state := xsEscape_amp;
            'q':
              FStack^.state := xsEscape_quot;
            '#':
      ...
      xsEscape_char_num:
        begin
          ...
          case AnsiChar(c) of
            '0'..'9':
              FPosition := (FPosition * 10) + (SOIChar(c) - 48);
            ';':
              begin
...
begin
  ...
  ecp := so('{iso-8859-1:   28591,' + 'iso-8859-2:   28592,' + 'iso-8859-3:   28593,' + 'iso-8859-4:   28594,' + 'iso-8859-5:   28595,' + 'iso-8859-6:   28596,' + 'iso-8859-7:   28597,' + 'iso-8859-8:   28598,' + 'iso-8859-9:   28599,' + 'iso 8859-15:  28605,' + 'iso-2022-jp:  50220,' + 'shift_jis:    932,' + 'euc-jp:       20932,' + 'ascii:        20127,' + 'windows-1251: 1251,' + 'windows-1252: 1252}');
