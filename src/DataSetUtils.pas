...
class function TDataSetUtils.CreateField(DataSet: TDataSet; FieldType: TFieldType; const FieldName: string; ASize: Integer; ADisplayWidth: Integer): TField;
begin
  ...
  if (FieldType = ftString) and (ASize <= 0) then
    raise Exception.CreateFmt('Size mismatch for field "%s".', [FieldName]);
end;
